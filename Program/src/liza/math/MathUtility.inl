...
//------------------------------------------------
// ポリゴンの頂点と対象の座標から法線と高さを求める
//------------------------------------------------
template<typename T>
float GetHeightPolygon(
	const Vector3<T>& p0,
	const Vector3<T>& p1,
	const Vector3<T>& p2,
	const Vector3<T>& pos,
	...
	Vector3<T> vec0 = p1 - p0;
	Vector3<T> vec1 = p2 - p0;
	Vector3<T> vecP = pos - p0;

	// 面法線求めるんだよな
	Vector3<T> cross = vec0.Cross( vec1 );
	Vector3<T> nor;
	nor.Normalize();
	...
	return p0.y - ( ( nor.x * vecP.x ) + ( nor.z * vecP.z ) ) / nor.y;
}
